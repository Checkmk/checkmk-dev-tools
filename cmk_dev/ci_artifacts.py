...
def download_artifacts(
    ...
    if len(artifact_hashes) > MAX_SINGLE_FILE_DOWNLOADS:
        ...
        # Several instances might download different builds into the same @out_dir concurrently
        # (e.g. parallel Jenkins stages sharing one workspace). With fixed names those instances
        # would write to the same archive and extraction directory, resulting in corrupted
        # archives and lost artifacts, so keep both unique per invocation.
        with TemporaryDirectory(dir=out_dir, prefix=TEMP_DIR_PREFIX) as tmp_dir:
            archive = _download_compressed_artifacts(
                client=client,
                build=build,
                out_dir=Path(tmp_dir),
                total_download_timeout=total_download_timeout,
            )
            downloaded_artifacts = _decompress_artifacts(artifact=archive, out_dir=out_dir)
    else:
...
def _download_compressed_artifacts(
    ...
    """Downloads all artifacts of @build as one compressed archive into @out_dir and returns the
    path of that archive"""
...
def _decompress_artifacts(
    ...
    # the archive is extracted next to itself into a directory unique per invocation, which gets
    # removed afterwards in any case
    with TemporaryDirectory(dir=artifact.parent, prefix=TEMP_DIR_PREFIX) as tmp_dir:
    ...
    if isinstance(args, Args) and args.use_influxdb:
        ...
        for this_build in reversed(matching_builds):
            ...
            try:
                ...
                log().warning(
                    "identify_matching_build() skipping build_number %s due to caught %r", build_number, e.errors()
                )
