...
class StageInfo(PedanticBaseModel):
    ...
    # _type: str = "StageInfo"
    type: Literal["StageInfo"] = "StageInfo"
...
class AugmentedJenkinsClient:
    ...
    @async_retry(tries=MAX_ATTEMPTS, delay=1, logger=log())
    async def raw_run_info(self, job: str | Sequence[str]) -> GenMap:
        job_path = job.split("/") if isinstance(job, str) else job
        url = f"{'/job/'.join((self.client.server, *job_path))}/wfapi/runs"
        log().debug("fetch run information for %s", job_path)
        self._api_call_count += 1
        return cast(GenMap, self.client._session.get(url).json())

    async def stages(self, job: Sequence[str]) -> Mapping[int, Sequence[StageInfo]]:
    ...
    async def get_artifact(self, build: Build, name: str) -> None:
        with self.client._session.get(f"{build.url}artifact/{name}", stream=True) as reply:
        ...
        if "queue" in show:
            ...
            for simple_queue_item in queue_items:
                ...
                rich_print(
                    ...
                    f" blocked: {str(qi.blocked):>5}"
                    f" buildable: {str(qi.buildable):>5}"
                    f" cancelled: {str(qi.cancelled):>5}"
                    f" stuck: {str(qi.stuck):>5}"
                    f" pending: {str(qi.pending):>5}"
        ...
        if "all_jobs" in show:
            ...
            async for node_path, node in jenkins_client.traverse_job_tree(ignored_pattern=None):
                ...
                for build_nr in sorted((b.number for b in job_info.builds), reverse=True):
                    ...
                    print(
                        f"{job_info.path} {build_nr} {build_info.timestamp} {build_info.duration}"
                    )
