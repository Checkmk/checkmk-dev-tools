...
def test_filter_by_prefix() -> None:
    ...
    assert filter_by_prefix(dictionary=single_entry_dict, unallowed_prefixes=["asdf_", "qwertz_"], strip_prefix="") == single_entry_dict
    ...
    assert filter_by_prefix(dictionary=multi_entry_dict, unallowed_prefixes=[f"{other_prefix}_"], strip_prefix="") == single_entry_dict
    ...
    assert filter_by_prefix(dictionary=multi_entry_dict, unallowed_prefixes=[f"{x}" for x in single_entry_dict.keys()], strip_prefix=f"{other_prefix}_") == other_entry_dict
...
def test_extract_credentials_from_file() -> None:
    ...
    assert extract_credentials(credentials_file=str(jenkins_jobs_file), config_section="influxdb_testing") == influxdb_testing_creds
    assert extract_credentials(credentials_file=str(jenkins_jobs_file), config_section="influxdb") == influxdb_creds
...
def test_extract_credentials_from_env(mock_settings_env_vars: Generator[Dict[str, Any], None, None]) -> None:
    credential_args = {
        "url_env": "JENKINS_URL",
    ...
    assert extract_credentials(credentials=credential_args, config_section="influxdb") == influxdb_creds
